...
!(/** @lends DJT*/function() {
     ...
     *
     * @param {number|string|DJT~Tpa} [initial-value] Initial value to set this number.
     ...
     * __Note well:__ If initial-value is itself a Tpa, then the same Tpa is returned *without* making a copy when called statically.
     * @param {boolean} [is-integer=true] Set to `false` to enable this number to represent fractions.
     * If the initial-value is fractional in any way then is-integer will default to `false`.
     * The initial setting of this number (integer or fractional) is always kept throughout its life unless
     * the Tpa~makeFraction or Tpa~makeInteger methods are called to change it.
     *
     * @example
    ...
    var N=(/** @lends TPA */function() {
        ...
        /**
         * @returns {number} 0 if number is zero, -1 if negative, +1 if positive
         */
        ...
        N.prototype.random=function(digits) {
            ...
            this.digits[i]=(1+Math.trunc(Math.random() * Math.pow(10, digits)));
            this.safemaximum=BASE-1;
            return this;
        };
     ...
     * @returns {Tpa} This number for chaining purposes
     */
    Tpa.prototype.mod=function(number) {
        if (!(number instanceof TPA)) number=Tpa(number);
    ...
    Tpa.prototype.toString=function(maxdp) {
        if (typeof maxdp != 'number' || isNaN(maxdp)) {
            if (arguments.length>0) throw new Error('toString() takes an optional parameter to specify the maximum DPs to output [default=100] (0=limitless)');
            else maxdp=100;
        }
